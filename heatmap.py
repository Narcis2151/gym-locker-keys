...
def create_heatmap(image_text_path, nr_lockers=180):
    image_text = pd.read_csv(image_text_path)

    # Create a DataFrame for lockers
    locker_df = pd.DataFrame({"Locker": range(1, nr_lockers + 1), "Count": 0})

    # Count the number of times each locker is used
    for index, row in locker_df.iterrows():
        locker = row["Locker"]
        locker_df.loc[index, "Count"] = image_text[image_text["text"] == locker].shape[
            0
        ]
